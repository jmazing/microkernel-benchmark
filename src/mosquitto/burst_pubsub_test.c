...
// Define USE_RT to enable real-time priority boosts on both QNX and Linux
// Compile with -DUSE_RT for RT-equipped mode, or without for baseline mode

#define BROKER_CMD   "mosquitto"
#define CONF_FILE    "mosquitto.conf"
#define BROKER_HOST  "127.0.0.1"
#define TOPIC        "test/topic"
#define COUNT        10000       // Number of burst messages
...
// Helper: monotonic timestamp in ns
...
// Subscriber callback\ nstatic void on_message(struct mosquitto *mosq, void *obj, const struct mosquitto_message *msg) {
    if (recv_count < COUNT) {
...
int main() {
    // Lock memory to prevent paging jitter
    ...
    run_burst_test("Burst pub/sub (baseline or RT)");
